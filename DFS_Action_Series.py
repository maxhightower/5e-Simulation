...
def rule_shield(sequence, next_num, acting_entity):
    if next_num == 13: # don shield
        donning_shield = False
        if [x for x in acting_entity.equipped_armor if x.type == 'shield'] == []:
            donning_shield = True
        if donning_shield == True:
            return False
        
        for item in acting_entity.off_hand:
            if item.type == 'shield':
                return False
...
def rule_standing_up_rules(sequence, next_num, acting_entity):
    ...
    if prone == False and next_num == 20:
        return False
